...
class TestData {
  ...
  def createTeachers {
    for (i <- 1 to 30) {
      User.create.address(randomName(10)).phone(randomNumbers(9)).firstName(randomName(9))
        .secondName(randomName(9)).
  ...
  def createClasses() {
    ...
    for (i <- 0 to 11) {
      ClassModel.create.level((i / 4) + 1).descript(randomName(7)).division(dep(i % 4)).teacher(teachers.head.id.get).save
      teachers = teachers.drop(2)
    }
  }

  def createPupils() {
    ClassModel.findAll.foreach(cl => {
      for (i <- 1 to 20) {
        ...
        val father = User.create.firstName(randomName(9)).secondName(randomName(9)).lastName(lastName).birthDistrict("pomorskie").
        ...
        val mather = User.create.firstName(randomName(9)).secondName(randomName(9)).lastName(lastName).birthDistrict("pomorskie").
          address(address).birthPlace("Gdańsk").pesel(randomNumbers(11)).phone(randomNumbers(9)).email(randomString(10) + "@edu.org").
          birthDate(new Date(dateL - 1500000000000L - randomLong(10000000000L))).role("r")
        mather.save

        User.create.firstName(randomName(9)).secondName(randomName(9)).lastName(lastName).birthDistrict("pomorskie").classNumber(i).
          address(address).birthPlace("Gdańsk").pesel(randomNumbers(11)).phone(randomNumbers(9)).email(randomString(10) + "@edu.org").
          classId(cl.id.get).classInfo(cl.classString()).birthDate(new Date(dateL - 500000000000L - randomLong(10000000000L))).role("u")
          .father(father.id.get).mather(mather.id.get).save
      }
    })
  }
