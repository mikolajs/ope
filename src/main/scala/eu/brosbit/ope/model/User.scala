...
import Mailer._
import eu.brosbit.ope.api.ConfigLoader
...
object User extends User with MetaMegaProtoUser[User] {
  ...
  override def emailFrom: String = ConfigLoader.emailAddr
  ...
  ///THIS OVERRIDED NOT WORK!!!!!!!!!!!!!!!
  /*
  override def signupMailBody(user:User, validationLink:String) = {
    val httpsLink = toHTTPSLink(validationLink)
    println("validationLinkNew: " + httpsLink)
    super.signupMailBody(user, httpsLink)
  }

  private def toHTTPSLink(link: String): String = {
    val linkArr = S.hostAndPath.split(':')
    val newLink = if (linkArr.length > 0 && linkArr.head == "http") linkArr.head + "s:" + linkArr.drop(1).mkString
    else link
    println("Generated new https " + newLink)
    newLink
  }

  override def generateValidationEmailBodies(user: User, resetLink: String):
  List[MailBodyType] = List(xmlToMailBodyType(signupMailBody(user, toHTTPSLink(resetLink))))
  */
}
