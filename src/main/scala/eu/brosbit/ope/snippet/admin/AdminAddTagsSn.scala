...
package eu.brosbit.ope.snippet.admin

//import java.util.Date
//import scala.xml.{NodeSeq, Text, XML, Unparsed}
import _root_.net.liftweb.util._
//import _root_.eu.brosbit.osp.lib.MailConfig
...
class AdminAddTagsSn {
  ...
  def indexTags: CssSel = {
    def mkIndex():Unit = {
      ...
      NewsTag.updateMany(Nil, "$set" -> ("count" -> 0))
      map.keys.foreach(key => {
        NewsTag.updateMany("tag" -> key, "$set" -> ("count" -> map(key)))
      })
    }
  ...
  def tagsList: CssSel = {
    ...
    "tr" #> newsTags.map(newsTag => {
      <tr>
        <td>
          {newsTag.tag + " (" + newsTag.count.toString + ")"}
        </td>
      </tr>
    })
  }
