...
class AdminEditMenuSn {
  ...
  val departs: Seq[(String, String)] = PageDepartment.findAll(Nil, "nr"->1).map(pd => (pd._id.toString -> pd.name))
  def addLinks(): CssSel = {
    var xmlDataStr = ""
    def add(): Unit = {
      MainPageMenu.drop
  ...
  private def createDepartMenu(dep: String, name:String): Unit = {
    PageDepartment.find(dep).map(pd => {
      ...
      mpm.links =  Link( "/page/" + pd._id.toString , "Aktualności")::links
      mpm.save
    })
  ...
  //change to stateful menu with all content of apps in /login
