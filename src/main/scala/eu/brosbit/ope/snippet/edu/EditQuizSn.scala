...
class EditQuizSn extends BaseResourceSn {
  ...
  private def mkExamDiv(ex: Exam) = {
    ...
    <div class="col-lg-4">
      ...
      <p>Grupa:
        ...
        rzędy:
        <strong>
          {ex.quizzes.length.toString}
        </strong>
      </p>
      <p>
        <em>Start:</em>{Formater.strForDateTimePicker(new Date(ex.start))}<br/>
        <em>Koniec:</em>{Formater.strForDateTimePicker(new Date(ex.end))}
      </p>
      <p>
        <a href={"/educontent/showexams/" + ex._id.toString}>
          <span
          class="btn btn-info">
            <span
        ...
        <a href={"/educontent/editexam/" + ex._id.toString}>
          <span
  ...
  private def mkBox(quest: QuizQuestion, p: Int) = {
    <li id={quest._id.toString}>
      ...
      <div class="questInfo">
        <span class="nr">
          {Formater.mkLongExerciseNumber(quest.nr)}
        </span>
        |
