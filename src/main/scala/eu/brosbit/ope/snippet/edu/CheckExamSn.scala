...
import Helpers._
import net.liftweb.util.CssBind._
...
class CheckExamSn {
      ...
      <div class="panel panel-info">
        <div class="panel-heading">
          ...
          <input type="number" min="0" maxlength="5" max={pktA.toString} name={"zad_" + nr.toString}
                 value={pktG.toString} onchange="checkExam.reCountData()"/>
        </div>
        <div class="panel-body">
          <div class="questionText">
            {Unparsed(question.question)}
          </div>{createAnswers(question.answers, question.fake, ans)}
        </div>
      </div>
    </section>
  }
  ...
  protected def createAnswers(good: List[String], fake: List[String], ans: String) = {
    if (fake.length == 0) {
      if (good.length > 0) {
        ...
        <div class={"answerBox alert" + addC} name="one">
          {Unparsed(ans)}
        </div>
      }
      else <div name="open" class="well">
        <pre>
          {ans}
        </pre>
      </div>
      ...
      val all = fake.map(s => {
        ...
        <li class={"list-group-item " + col}>
          <span class={"glyphicon" + addC}></span>{Unparsed(s)}
        </li>
      }) ++
        good.map(s => {
          val isAns = ansList.exists(_ == s)
  ...
  private def createFromJsonList(jsonStr: String) = {
    ...
    try {
      ...
      case _: Throwable => println("ERROR PARSER JSON PERFORMEXAMS")
    }
    data
  }
