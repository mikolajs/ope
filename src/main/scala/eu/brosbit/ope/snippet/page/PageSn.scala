package eu.brosbit.ope.snippet.page
...
class PageSn extends FlashTileSn {
  ...
  val user: Box[User.TheUserType] = User.currentUser
  val isTeacher: Boolean = if (user.isEmpty) false
  else {
  ...
  val dep: String = S.param("id").openOr("0")
  private val pageDep = PageDepartment.find(dep).getOrElse(PageDepartment.create)
  val what: String = S.param("w").getOrElse("n") //all news by default
  ...
  private def showNewses(newses: List[ArticleHead]) = {
    val sizeP = 10
    ...
    "#articleCont" #> "" &
      "#departmentInfo *" #> (if(newses.nonEmpty) newses.head.departmentName else "") &
      ".newsInfo" #> <div>
