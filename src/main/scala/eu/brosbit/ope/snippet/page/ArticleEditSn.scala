...
import _root_.scala.xml.Null
import _root_.net.liftweb.util._
import _root_.net.liftweb.common._
import _root_.eu.brosbit.ope.model._
...
class ArticleEditSn {
  ...
  private val pageTypes = Map(true -> "Aktualności", false -> "Artykuły")
  private val thumbDefault = "/images/nothumb.png"
  ...
  private def deleteObjectById(id: String): Unit = {
    ArticleHead.find(id) match {
      case Some(articleHead) => {
        val articleContentOpt = ArticleContent.find(articleHead.content)
        if (articleContentOpt.isDefined) articleContentOpt.get.delete
  ...
  private def updateNewsTags(toAddTags: List[String], toDeleteTags: List[String]): Unit = {
